...
export default function Page() {
  ...
  const handleEmailSubmit = async () => {
    ...
    if (!email.includes("@") || !email.split("@")[0] || !email.endsWith("@defence.gov.sg")) {
      setToast({ message: "Please enter a valid @defence.gov.sg email address.", type: "error" });

      return;
  }

    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/wordplay-otp?email=${encodeURIComponent(email)}&otp=none`, {
        method: 'GET',
          headers: {
              'Content-Type': 'application/json'
          }
  ...
  const handleOtpVerification = async () => {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/wordplay-otp?email=${encodeURIComponent(email)}&otp=${encodeURIComponent(otp)}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json'
        }
    ...
    if (response.status === 200) {
        ...
        setToast({ message: "Invalid or expired OTP. Please try again.", type: "error" });
    }
};
  ...
  const handleAdminLogin = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/wordplay-admin-password`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
...
return (
  <div>
    ...
    {showResults && filteredData.map((item) => (
...
<a onClick={() => setAdminLoginModalOpen(true)}>
        Admin Portal
        {pendingCount > 0 && <span className="notification-bubble">{pendingCount}</span>}
    </a>
</div>
        ...
        <div className="modal-fields-container">
          <div className="modal-content">
            ...
            <div className="modal-buttons">
              {!otpSent ? (
                <button onClick={handleEmailSubmit} className="modal-submit-button">Get OTP</button>
              ) : (
                <button onClick={handleOtpVerification} className="modal-submit-button">Verify OTP</button>
              )}
              <button onClick={() => setisUserLoginModalOpen(false)} className="modal-cancel-button">Cancel</button>
            </div>
          </div>
        </div>
      )}
...
{isAdminLoginModalOpen && (
  <div className="modal-fields-container">
    <div className="modal-content">
      ...
      />
      <div className="modal-buttons">
        <button onClick={handleAdminLogin} className="modal-submit-button">Login</button>
        <button onClick={() => setAdminLoginModalOpen(false)} className="modal-cancel-button">Cancel</button>
      </div>
    </div>
  </div>
)}
